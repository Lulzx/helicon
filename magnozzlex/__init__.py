"""MagNozzleX — GPU-Accelerated Magnetic Nozzle Simulation Toolkit."""
...
def run(config, output_dir=None, dry_run=False, **kwargs):
    """Run a MagNozzleX simulation.

    Thin wrapper around :func:`magnozzlex.runner.launch.run_simulation`.
...
def postprocess(output_dir) -> Metrics:
    ...
    Calls :func:`magnozzlex.postprocess.report.generate_report` and wraps
    the result into a :class:`Metrics` object.

    Parameters
    ----------
    output_dir : path-like
        Directory containing WarpX/MagNozzleX output files.
